...
- **Cores e estilo:** as cores principais estão definidas como variáveis CSS no início de `css/style.css`. As principais são:
  - `--primary` — cor principal (azul padrão: `#1a73e8`)
  - `--primary-dark` — variação escura da cor principal
  - `--secondary` — cor de destaque/acento (laranja padrão: `#ff6b35`)
  - `--text-dark` — cor do texto principal
  - `--bg-light` — cor de fundo das seções claras
